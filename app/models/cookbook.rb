...
class Cookbook
  ...
  def with_urls
    ...
    m.generate_manifest_with_urls { |o| puts Chef::CookbookVersion.cookbook_file_for_checksum(o[:checksum]) }
  end
  ...
  def metadata
    ...
    @settings[:metadata_filenames].each do |meta_json|
      begin
        metadata.from_json((repo.tree(@branch)/'cookbooks'/meta_json).data)
  ...
  def versioner
    branch = @branch
    cookbook = @cookbook
    tree = repo.tree(branch) / 'cookbooks' / cookbook
